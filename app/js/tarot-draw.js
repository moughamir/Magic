...
$(document).ready(function () {
    ...
    cardsPath = (typeof cardsPath !== "undefined") ? cardsPath : 'app/images/major/';
    trt_deckNbCards = (typeof trt_deckNbCards !== "undefined") ? trt_deckNbCards : 22;
    nbCardsToDraw = (typeof nbCardsToDraw !== "undefined") ? nbCardsToDraw : 3;
    ...
    $(document).on('click', '#cards-container .notFlipped', function (e) {
        if (!trt_lock) {
            ...
            $('form').append('<input type="hidden" name="{{card.value}}" value="' + cardNumber + '.png" />');
            ...
            setTimeout(function () {
                ...
                if ($(window).width() > trt_minSize) { // si version grand format
                    ...
                    card.animate({top: position + 'px', left: position + 'px'}, 500);
                } else {
                ...
                setTimeout(function () {
                    ...
                    place.css({
                        'background-image': 'url(' + cardsPath + cardNumber + '.png)',
                    ...
                    if (currCardCounter === nbCardsToDraw) {
                        ...
                        if (trt_scrollOnComplete) {
                            $('html,body').animate({scrollTop: $('#form-container').offset()}, 1000);
                        }
                    }
                }, 300);
        ...
        if ($(window).width() > trt_minSize) {
            for (var i = 1; i <= 3; i++) {
                ...
                if (position !== undefined) {
                    $('#cards-container .Flipped[data-counter="' + i + '"]').css({
                        position: 'absolute',
                        top: position.top + 'px',
